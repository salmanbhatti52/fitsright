// ignore_for_file: avoid_print, unused_element, file_names, empty_catches, use_rethrow_when_possible

import 'dart:convert';
import 'package:dio/dio.dart';
import 'package:fits_right/utils/api_urls.dart';
...
abstract class DioService<T> {
  ...
  static post(String route, data, [Map<String, dynamic>? customHeaders, Function(String)? onProgress]) async {
    ...
    try {
      print("$baseUrl$route");
      Response res = await dio.post('$baseUrl$route', data: data, options: Options(
  ...
  static patch(String route, data) async {
    ...
    try {
      Response res = await dio.patch("$baseUrl$route",
  ...
  static get(String route) async {
    Dio dio = Dio();
    try {
      Response res = await dio.get("$baseUrl$route");
      print("$baseUrl$route");
      return res.data;
    } on DioError catch (e) {
      if (e.response!.statusCode == 401) {
  ...
  static delete(String route, data) async {
    Dio dio = Dio();
    try {
      Response res = await dio.delete("$baseUrl$route",
          data: data, options: Options(headers: headers));
      print("$baseUrl$route");
      return res;
    } on DioError catch (e) {
